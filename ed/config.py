...
# ----------------------------------------------------------------------------
# Sz-sector factorization
# ----------------------------------------------------------------------------
# The hopping Hamiltonian is spin-diagonal (t1, t2, phi, and the twisted
# boundary flux are identical for both spins -- see hamiltonian.py) and the
# interaction terms (U, V, staggered potential) are all density-density, so
# total Sz = 1/2 * sum_i (n_i_up - n_i_dn) is a good quantum number on top of
# total N. Rather than diagonalizing over the full N-electron space spanning
# every Sz sector (dimension C(24,12) = 2,704,156), we fix N_UP and N_DN
# separately and only ever build the requested Sz sector (default: the
# physically relevant Sz=0 half-filled sector, dimension
# C(12,6) x C(12,6) = 854,076 -- a 3.2x reduction on top of everything else).
#
# Change these if you want a different (still spin-diagonal-Hamiltonian)
# sector, e.g. a doped/polarized case with N_UP != N_DN.
N_UP = NUM_ELECTRONS // 2
N_DN = NUM_ELECTRONS - N_UP
assert N_UP + N_DN == NUM_ELECTRONS, "N_UP + N_DN must equal NUM_ELECTRONS"
